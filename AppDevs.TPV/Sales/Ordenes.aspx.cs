...
namespace AppDevs.TPV.Sales
{
    public partial class Ordenes : System.Web.UI.Page
    {
        ...
        static void ImprimirPedidos(IGrouping<bool, SPC_GET_ORDENDETALLE_Result> Pedidos, int Comensales, bool ImprimirEnCocina,
            string NombreImpresoraCocina, string NombreImpresoraBarra)
        {
            try
            {
                ...
                Boolean? primeros = Pedidos.First().Primero;
                foreach (var pedido in Pedidos)
                {
                    if (primeros != pedido.Primero)
                    {
                        primeros = pedido.Primero;
